...
jQuery(document).ready(function () {
    ...
    var controlGoogleAnalytics = dataLayer.push({'pcfkLovers': 'Paso 0'});
    ...
    jQuery('.on-want').click(function () {
        ...
        var registro = '';
        registro += '<tr data-precioped="' + precio + '" data-cantidadped="1" data-talla="' + talla + '" data-plu="' + plu + '" data-product_id="' + product_id + '">';
        ...
        //MENSAJE EXITO
        mostrarPopup('success', langpcfk.productoSeleccionado);
        ...
        if (jQuery(".wpcf7-form").valid()) {
            ...
            jQuery("#tablepress-1 tbody.row-hover tr").each(function () {
                if (typeof jQuery(this).data('plu') !== 'undefined' &&
                        ...
                        typeof jQuery(this).data('product_id') !== 'undefined') {
                    pedido += '||' + jQuery(this).data('plu') + '-' + jQuery(this).data('talla') + '-' + jQuery(this).data('cantidadped') + '-' + jQuery(this).data('product_id');
                }
            });
            var pedido = {
...
//                        jQuery('.modal-buttom-buy').hide();
    ...
    jQuery('.rigth > a.btn-buy').click(function () {
        ...
        if (selectedTab == 1) {
            jQuery(".jq-tabs").tabs("enable", 2);
            jQuery('.jq-tabs').tabs("option", "active", 2);
            if (jQuery.isEmptyObject(infoProfileFace)) {
                jQuery('.rigth > a.btn-buy').hide();
            } else {
                jQuery('.rigth > a.btn-buy').show();
                jQuery('.rigth > a.btn-buy').html('Finalizar');
                jQuery('.rigth > a.btn-buy').attr('onclick', 'enviarForm()');
            }
    ...
    jQuery('.left > a.btn-pre').click(function () {
        var selectedTab = jQuery(".jq-tabs").tabs('option', 'active');
        jQuery(".jq-tabs").tabs("option", "active", selectedTab - 1);
        jQuery('.rigth > a.btn-buy').html('Siguiente');
        jQuery('.rigth > a.btn-buy').removeAttr('onclick');
        return false;
    });
    ...
    jQuery(".jq-tabs").on("tabsactivate", function (event, ui) {
        ...
        //controlGoogleAnalytics = 'Paso ' + parseInt(selectedTab + 1);
        if (selectedTab == 1) {
            controlGoogleAnalytics = dataLayer.push({'pcfkLovers': 'Paso 2'});
            ...
            controlGoogleAnalytics = dataLayer.push({'pcfkLovers': 'Paso 3'});
            if (jQuery.isEmptyObject(infoProfileFace)) {
                jQuery('.rigth > a.btn-buy').hide();
            } else {
                jQuery('.rigth > a.btn-buy').show();
            ...
            controlGoogleAnalytics = dataLayer.push({'pcfkLovers': 'Paso 1'});
            ;
            jQuery('.left > a.btn-pre').hide();
            jQuery('.rigth > a.btn-buy').show();
        } else {
            controlGoogleAnalytics = dataLayer.push({'pcfkLovers': 'Paso 4'});
            ;
            jQuery('.rigth > a.btn-buy, .left > a.btn-pre').hide();
        }
        ...
        //ESCONDO EL BANNER EN TODOS LOS PASOS
        ...
        jQuery.ajax({
            ...
            success: function (response) {
                if (!response.error) {
                    ...
                    //jQuery('.modal-buttom-buy').show();
                    //ESCONDO EL BANNER DESDE EL PASO 1
                    jQuery('#BannerMain').hide('slow');
                    jQuery('.tab-products-buy').addClass('margtop');
                    jQuery('.logo').addClass('logot');
                    jQuery("html, body").stop().animate({
    ...
    function construirTablaPedidos() {
        ...
        //CAMBIO EL PRECIO        
        jQuery('#carttotal').html('$' + addCommas(preciopedido));
...
/**
 * Metodo para hacer submit del formulario con el boton Finalizar
 *
 * @author Felipe Echeverri <felipe.echeverri@ingeneo.com.co>
 * @copyright 2017 Linea Directa
 */
...
function statusChangeCallback(response) {
    ...
    if (response.status === 'connected') {
        ...
        //jQuery('input#checkoutsubscribe').show();
        jQuery('input#checkoutsubscribe').hide();
        jQuery('.faceconnect').hide();
        jQuery('.finaliza').hide();
        //MUESTRO BOTON CON NOMBRE FINALIZAR Y FUNCION PARA HACER SUBMIT
        jQuery('.rigth > a.btn-buy').show();
        jQuery('.rigth > a.btn-buy').html('Finalizar');
        jQuery('.rigth > a.btn-buy').attr('onclick', 'enviarForm()');
    } else {
        jQuery('input#checkoutsubscribe').hide();
        jQuery('.finaliza').show();
        //MOSTRAR POPUP ERROR
        mostrarPopup('error', langpcfk.errorLoginFacebook);
    }
}

function varGoogleAnlytics(paso) {
    return paso;
}